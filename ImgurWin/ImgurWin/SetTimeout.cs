...
namespace ImgurWin
{
	public static class SetTimeout
	{
		...
		/// <param name="Timeout">Values less than 0 are treated as 0. If the Timeout value is 0 for an OnGui operation, the invoke process will start before this method returns, making it impossible to cancel the operation.</param>
		/// <returns></returns>
		...
		/// <summary>
		/// Invokes a call to SetTimeout.OnBackground on the Gui Thread, so that other UI events have a chance to finish first. This timeout will not be cancelable.
		/// </summary>
		/// <param name="TheAction">The action to run.</param>
		/// <param name="Timeout">Values less than 0 are treated as 0.</param>
		/// <returns></returns>
